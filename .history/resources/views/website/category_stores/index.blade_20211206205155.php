...
<div class="row featureds__item__text">
  <h5>All Stores</h5>
</div>
<div class="row">
...
<div class="row featureds__filter" id="MixItUpD9A4B2">
  ...
  {{-- {{$latestPaymentWithTrendingProduct}} --}}
    ...
    <div class="featureds__item">
      <div class="featureds__item__pic set-bg" data-setbg="{{asset('app_uploads/'.$store->icon)}}"
        ...
        <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#exampleModal{{$store->id}}">
          Rate it ⭐ </button>
        <div class="d-flex align-items-end flex-column" style="height: 233px;">
          <div class="mt-auto p-2" style="background-color: whitesmoke;">
            {{$store->rate->count('rate')}}👤
            {{$store->rate->average('rate')}}⭐
          </div>
        </div>
      </div>
